...
class ClientDataLoader implements ApplicationRunner {
    ...
    private void seedCalibreWeb() {
        ...
        repository.save(RegisteredClient.withId(UUID.randomUUID().toString())
                ...
                .authorizationGrantType(AuthorizationGrantType.AUTHORIZATION_CODE)
                .authorizationGrantType(AuthorizationGrantType.REFRESH_TOKEN)
                .redirectUri("http://localhost:8083/login/auth-server/authorized")
                .redirectUri("http://127.0.0.1:8083/login/auth-server/authorized")
                .redirectUri("https://books.example.com/login/auth-server/authorized")
                .postLogoutRedirectUri("http://localhost:8083/")
                .postLogoutRedirectUri("https://books.example.com/")
                .scope(OidcScopes.OPENID)
                .scope(OidcScopes.PROFILE)
                .scope(OidcScopes.EMAIL)
                .clientSettings(ClientSettings.builder()
    ...
    private void seedDrawio() {
        ...
        repository.save(RegisteredClient.withId(UUID.randomUUID().toString())
                ...
                .redirectUri("https://draw.example.com/")
                .postLogoutRedirectUri("http://localhost:8888/")
                .postLogoutRedirectUri("https://draw.example.com/")
                .scope(OidcScopes.OPENID)
                .scope(OidcScopes.PROFILE)
                .scope(OidcScopes.EMAIL)
                .clientSettings(ClientSettings.builder()
