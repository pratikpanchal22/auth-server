...
class ClientDataLoader implements ApplicationRunner {
    ...
    private void seedCalibreWeb() {
        ...
        repository.save(RegisteredClient.withId(UUID.randomUUID().toString())
                ...
                .authorizationGrantType(AuthorizationGrantType.AUTHORIZATION_CODE)
                .authorizationGrantType(AuthorizationGrantType.REFRESH_TOKEN)
                .redirectUri("http://localhost:8083/login/nthnode/authorized")
                .redirectUri("http://127.0.0.1:8083/login/nthnode/authorized")
                .redirectUri("https://books.nthnode.com/login/nthnode/authorized")
